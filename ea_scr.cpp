...
void final_edges()
{
    ...
    //cout << "here" << endl;
    //for (int i = 0; i < add_edges.size(); i++) cout << add_edges[i].first << " " << add_edges[i].second << endl;
}
...
void display(){
	...
	****************************/
    ...
    {
        point tp;
        tp.x = spos[i].x + (epos[i].x - spos[i].x) * aT;
        tp.y = spos[i].y + (epos[i].y - spos[i].y) * aT;
        locs.push_back(tp);
    }

    glColor3f(1, 0, 0);
    for (i = 0; i < locs.size(); i++) drawSquare(locs[i].x, locs[i].y, 1);
...
void animate(int val){
	...
	if (start_anim == true && aT < 1.0) aT += 0.05;
