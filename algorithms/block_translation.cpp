...
point scale(point p)
{
    ...
    p.x = p.x * (((len - cr) / len) + eps);
    p.y = p.y * (((len - cr) / len) + eps);
    return p;
}
...
void process_leaves()
{
    for (int i = 0; i < leaves.size(); i++)
    {
		...
		//something here
        int lb = leaves[i];
        if ((int)DT[lb].size() == 0) continue;
        int cv = DT[lb][0].first;

        if ((int)DT[cv].size() == 0) continue;
